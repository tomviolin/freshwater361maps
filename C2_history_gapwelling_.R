...
alldata = fread("fixedhistory.csv")# Read in the ship data
gap_kml = sf::st_read("gap_area.kml")
bb = as.data.frame(st_bbox(gap_kml$geometry))
...
map_bbox = sf::st_bbox(c(
  xmin=min(data$GPS_Long)-0.01,
  ymin=min(data$GPS_Lat)-0.01,
  xmax=max(data$GPS_Long)+0.01,
  ymax=max(data$GPS_Lat)+0.01),crs=4623)


#get tiles
tiles_map <- get_tiles(x = map_bbox, provider = "OpenStreetMap.DE",zoom=13, crop=TRUE)
